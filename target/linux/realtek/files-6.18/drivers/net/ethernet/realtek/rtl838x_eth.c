...
#define RTETH_STACK_CPU_BROADCAST	0xf8
					...
					int prio)
{
	...
	frag->cpu_tag[4] |= FIELD_PREP(RTL931X_CPU_TAG4_SW_UNIT, device);
}
				     ...
				     unsigned int target)
{
	...
	u16 fwd_type = mode == RTL838X_ETH_DEVICE_TALK_ONE_HOP ?
		       RTL93XX_CPU_TAG1_FWD_ONE_HOP :
		       RTL93XX_CPU_TAG1_FWD_UCST_CPU_MIN_PORT;

	frag->cpu_tag[0] = 0x8000;
	frag->cpu_tag[1] = FIELD_PREP(RTL93XX_CPU_TAG1_FWD_MASK, fwd_type) |
			   RTL93XX_CPU_TAG1_BYPASS_FILTER |
			   RTL93XX_CPU_TAG1_IGNORE_STP_MASK |
			   RTL93XX_CPU_TAG1_BYPASS_VLAN_EGR;
	frag->cpu_tag[4] = mode == RTL838X_ETH_DEVICE_TALK_UNICAST ?
		FIELD_PREP(RTL931X_CPU_TAG4_SW_UNIT, target) : dpm >> 48;
...
static bool rteth_930x_decode_tag(struct rteth_frag *frag, struct dsa_tag *t)
{
...
static bool rteth_931x_decode_tag(struct rteth_frag *frag, struct dsa_tag *t)
{
	u16 spn = frag->cpu_tag[2] & RTL931X_CPU_TAG2_SPN;

	t->stack_port = (frag->cpu_tag[0] >> 8) & 0x3f;
	t->device = FIELD_GET(RTL931X_CPU_TAG2_SPN_DEVICE, spn);
	t->port = FIELD_GET(RTL931X_CPU_TAG2_SPN_PORT, spn);
	t->queue = (frag->cpu_tag[2] >> 11) & 0x1f;
	t->reason = frag->cpu_tag[7] & 0x3f;
	t->crc_error = frag->cpu_tag[1] & BIT(6);
	t->l2_offloaded = (t->reason >= 19 && t->reason <= 27) ? 0 : 1;
...
int rtl838x_eth_stack_cpu_xmit(struct net_device *dev, struct sk_buff *skb)
{
	...
	trailer = skb_put(skb, RTETH_DEVICE_TALK_TRAILER_LEN);
	trailer[0] = RTETH_STACK_CPU_BROADCAST;
	trailer[1] = 0;
	trailer[2] = RTETH_DEVICE_TALK_MAGIC0;
	trailer[3] = RTETH_DEVICE_TALK_MAGIC1;
	trailer[4] = RTETH_DEVICE_TALK_MAGIC2;
...
EXPORT_SYMBOL_GPL(rtl838x_eth_stack_cpu_xmit);
...
enum rteth_destination_type {
	...
	RTETH_DESTINATION_STACK_CPU_BROADCAST,
};
				  ...
				  struct rteth_destination *destination)
{
	...
	if (ctrl->r->get_cpu_device && netdev_uses_dsa(dev) &&
	    trailer[2] == RTETH_DEVICE_TALK_MAGIC0 &&
	    trailer[3] == RTETH_DEVICE_TALK_MAGIC1 &&
	    trailer[4] == RTETH_DEVICE_TALK_MAGIC2) {
		if (trailer[0] == RTETH_STACK_CPU_BROADCAST && !trailer[1]) {
			destination->type =
				RTETH_DESTINATION_STACK_CPU_BROADCAST;
			return true;
		}

		if (skb->protocol != htons(ETH_P_802_EX1))
			return false;

		if (trailer[0] == RTETH_DEVICE_TALK_ONE_HOP &&
		    trailer[1] < ctrl->r->cpu_port) {
			destination->type =
				RTETH_DESTINATION_DEVICE_TALK_ONE_HOP;
...
static int rteth_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (destination.type == RTETH_DESTINATION_PHYSICAL &&
		...
		if (ctrl->r->get_cpu_device) {
			int device = ctrl->r->get_cpu_device(ctrl);
	...
	*packet_skb = skb;
	...
	if (destination.type == RTETH_DESTINATION_PHYSICAL)
		ctrl->r->create_tx_header(frag, destination.device, port, 0); // TODO prio 0?
		...
		rteth_931x_create_device_talk_header(frag,
						     ...
						     destination.device);
	else if (destination.type == RTETH_DESTINATION_STACK_CPU_BROADCAST) {
		frag->cpu_tag[0] = 0x8000;
		frag->cpu_tag[1] =
			FIELD_PREP(RTL93XX_CPU_TAG1_FWD_MASK,
				   RTL93XX_CPU_TAG1_FWD_BCST_CPU) |
			RTL93XX_CPU_TAG1_BYPASS_FILTER |
			RTL93XX_CPU_TAG1_IGNORE_STP_MASK |
			RTL93XX_CPU_TAG1_BYPASS_VLAN_EGR;
	}
...
static bool rteth_cpu_rx(struct rteth_ctrl *ctrl, struct sk_buff *skb,
			 int ring, u8 device, u8 ingress)
{
	struct rtl_otto_tagger_data *tagger_data;
	...
	read_lock_bh(&tagger_data->cpu_device_lock);
	stack_dev = tagger_data->stack_cpu_dev;
	if (tagger_data->cpu_device_changing ||
	...
	skb->protocol = eth_type_trans(skb, stack_dev);
	len = skb->len + ETH_HLEN;
	dev_sw_netstats_rx_add(stack_dev, len);
					...
					u8 *stack_port, u8 *stack_ingress)
{
	...
	*stack_ingress = 0;
	...
	if (netdev_uses_dsa(dev)) {
		if (ctrl->r->get_cpu_device && tag.reason == 1) {
			if (tag.crc_error || tag.stack_port >= ctrl->r->cpu_port) {
				*remote = true;
			} else {
		 ...
		 * metadata unless an active stack session identifies the ingress as
		 * fabric. Keep mismatches fenced across an ID handover.
		 */
		if (tag.device != local_device) {
			rcu_read_lock();
			if (local_device != U8_MAX && ctrl->r->get_cpu_device &&
			...
			rcu_read_unlock();
			   ...
			   rteth_931x_reject_local_mismatch(ctrl, local_device,
							    tag.stack_port)) {
			*remote = true;
		} else if (tag.port < ctrl->r->cpu_port) {
...
static int rteth_hw_receive(struct net_device *dev, int ring, int budget)
{
	...
	dropping = ctrl->rx_info[ring].dropping;
	device_talk = ctrl->rx_info[ring].device_talk;
	stack_frame = ctrl->rx_info[ring].stack_frame;
	stack_device = ctrl->rx_info[ring].stack_device;
	stack_port = ctrl->rx_info[ring].stack_port;
	stack_ingress = ctrl->rx_info[ring].stack_ingress;
	...
	while (work_done < budget) {
		...
		if (is_head) {
			...
			local_device = 0;
			if (ctrl->r->get_cpu_device) {
				local_device = ctrl->r->get_cpu_device(ctrl);
			...
			if (unlikely(skb->len < ETH_HLEN + ETH_FCS_LEN)) {
				...
				if (stack_frame) {
					...
					read_unlock_bh(&tagger_data->cpu_device_lock);
					   ...
					   htons(ETH_P_802_EX1)) {
					unsigned int stack_len = skb->len;

					if (rteth_cpu_rx(ctrl, skb, ring, stack_device,
							 stack_ingress)) {
						rx_bytes += stack_len;
						rx_packets++;
					} else {
					...
					if (device_talk) {
						...
						rx_bytes += skb->len;
						rx_packets++;
						napi_gro_receive(&ctrl->rx_info[ring].napi,
							 skb);
					}
				}
frame_done:
...
recycle:
	...
	ctrl->rx_info[ring].dropping = dropping;
	ctrl->rx_info[ring].device_talk = device_talk;
	ctrl->rx_info[ring].stack_frame = stack_frame;
	ctrl->rx_info[ring].stack_device = stack_device;
	ctrl->rx_info[ring].stack_port = stack_port;
	ctrl->rx_info[ring].stack_ingress = stack_ingress;
